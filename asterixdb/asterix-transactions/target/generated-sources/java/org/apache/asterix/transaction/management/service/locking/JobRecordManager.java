...
public class JobRecordManager {
    ...
    public static int LAST_WAITER_OFF = 8; // size: 8
    public static int LAST_UPGRADER_OFF = 16; // size: 8
    public static int NEXT_FREE_SLOT_OFF = 24; // size: 4
    public static int JOB_ID_OFF = 28; // size: 4
    public static int ALLOC_ID_OFF = 32; // size: 2
    ...
    public long getLastWaiter(int slotNum) {
        ...
        buf.checkSlot(slotNum % NO_SLOTS);
        final ByteBuffer b = buf.bb;
        return b.getLong((slotNum % NO_SLOTS) * ITEM_SIZE + LAST_WAITER_OFF);
    }

    public void setLastWaiter(int slotNum, long value) {
        final ByteBuffer b = buffers.get(slotNum / NO_SLOTS).bb;
        b.putLong((slotNum % NO_SLOTS) * ITEM_SIZE + LAST_WAITER_OFF, value);
    }
    ...
    public long getLastUpgrader(int slotNum) {
        ...
        buf.checkSlot(slotNum % NO_SLOTS);
        final ByteBuffer b = buf.bb;
        return b.getLong((slotNum % NO_SLOTS) * ITEM_SIZE + LAST_UPGRADER_OFF);
    }

    public void setLastUpgrader(int slotNum, long value) {
        final ByteBuffer b = buffers.get(slotNum / NO_SLOTS).bb;
        b.putLong((slotNum % NO_SLOTS) * ITEM_SIZE + LAST_UPGRADER_OFF, value);
    }
    ...
    static class Buffer {
        ...
        public int allocate() {
            ...
            bb.putLong(slotNum * ITEM_SIZE + LAST_WAITER_OFF, -1);
            bb.putLong(slotNum * ITEM_SIZE + LAST_UPGRADER_OFF, -1);
            bb.putInt(slotNum * ITEM_SIZE + NEXT_FREE_SLOT_OFF, -1);
            bb.putInt(slotNum * ITEM_SIZE + JOB_ID_OFF, 0xdeadbeef);
        ...
        StringBuilder append(StringBuilder sb) {
            ...
            for (int i = 0; i < NO_SLOTS; ++i) {
                long value = bb.getLong(i * ITEM_SIZE + LAST_WAITER_OFF);
                sb = TypeUtil.Global.appendFixed(sb, value);
                sb.append(" | ");
            }
            sb.append("\n");
            ...
            for (int i = 0; i < NO_SLOTS; ++i) {
                long value = bb.getLong(i * ITEM_SIZE + LAST_UPGRADER_OFF);
                sb = TypeUtil.Global.appendFixed(sb, value);
                sb.append(" | ");
            }
            sb.append("\n");
        ...
        private void checkSlot(int slotNum) {
            ...
            if (bb.getLong(itemOffset + LAST_WAITER_OFF) == 0xdeadbeefdeadbeefl) {
                String msg = "invalid value in field LAST_WAITER_OFF of slot " + TypeUtil.Global.toString(slotNum);
                throw new IllegalStateException(msg);
            }
            if (bb.getLong(itemOffset + LAST_UPGRADER_OFF) == 0xdeadbeefdeadbeefl) {
                String msg = "invalid value in field LAST_UPGRADER_OFF of slot " + TypeUtil.Global.toString(slotNum);
                throw new IllegalStateException(msg);
            }
